...
const ContactForm = () => {
  ...
  const validate = (form: { name: string, email: string, description: string }) => {
    ...
    if (form.description.split(' ').filter(word => word !== '').length === 0) newErrors.description = 'Please enter a description.';
    setErrors(newErrors);
    return newErrors;
  };
  ...
  return (
    ...
    <div>
      <label>
        Description:
        <textarea name="description" value={form.description} onChange={handleDescriptionChange} />
      </label>
